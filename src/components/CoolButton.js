...
import { ArrowRightIcon, CodeBracketIcon, 
        ...
        HomeIcon, EnvelopeIcon } from '@heroicons/react/24/outline'

export default function CoolButton(props) {

    return (
        <div className="pl-7 pt-3 text-xl">
            ...
            <span className="absolute left-0 pl-2.5 -translate-x-12 text-white group-hover:translate-x-0 ease-out duration-150">
                <ArrowRightIcon className="h-6 w-6" aria-hidden="true" />
            </span>
            <span className="relative w-full text-left transition-colors duration-200 ease-in-out group-hover:text-white">{props.buttonTitle}</span>
            </a>
        </div>
    )
}
...
function iconSelector(str) {
    if (str === "blog") {
        ...
        return <EnvelopeIcon className="h-6 w-6" aria-hidden="true" />;
    }
}
